...
func (pwm PWM) Configure(config PWMConfig) error {
	...
	pwm.Timer.WAVE.SetBits(sam.TCC_WAVE_WAVEGEN_NPWM)

	// Wait for synchronization of all changed registers.
	for pwm.Timer.SYNCBUSY.Get() != 0 {
	}

	// Set the period, possibly also setting the prescaler.
...
func (pwm PWM) Channel(pin Pin) (PWMChannel, error) {
	...
	// Set pin as output
	sam.PORT.DIRSET0.Set(1 << uint8(pin))
	// Set pin to low
	sam.PORT.OUTCLR0.Set(1 << uint8(pin))
